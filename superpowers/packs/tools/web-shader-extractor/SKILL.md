...
## Fusera Harness Notes

- Pack id: `tools/web-shader-extractor`.
- Source-root path: `superpowers/packs/tools/web-shader-extractor`.
- Use this as a tool pack for `web-shader-extraction`, not as part of the default landing-page artifact spine.
- In this repository, do not use Playwright unless the user explicitly asks for it or authorizes it for the current task.
- Prefer public pages or pages the user is authorized to reproduce. Do not persist cookies, Authorization headers, tokens, or secrets.
- Keep captures, screenshots, source slices, replay baselines, and generated project files in the user-provided output directory or run-owned runtime output, not in canonical `superpowers/` source.
- When this file is copied into a companion skills bundle, resolve `references/`, `scripts/`, and `templates/` from the authoritative Fusera `source_root`.
