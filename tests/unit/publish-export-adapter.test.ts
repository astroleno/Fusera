...
describe("publish/export external adapter contract", () => {
  ...
  it("records thrown adapter execution failures as external_failed", async () => {
    const throwingAdapter: PublishExportAdapter = {
      ...noopExportAdapter,
      execute: vi.fn().mockRejectedValue(new Error("External export crashed.")),
    };
    const transitionOperation = vi
      .fn()
      .mockResolvedValueOnce({
      ...
      .mockResolvedValueOnce({
        ...
        operation: operation("external_failed", "export", {
          externalResult: {
            ...
            message: "External export crashed.",
            details: {
              exception: true,
              phase: "execute",
              errorName: "Error",
  ...
  it("records thrown adapter normalization failures as external_failed", async () => {
    const throwingAdapter: PublishExportAdapter = {
      ...noopPublishAdapter,
      normalizeResult: vi.fn(() => {
        throw new Error("Malformed external response.");
      }),
    };
    const transitionOperation = vi
      .fn()
    ...
    expect(result).toMatchObject({
      ...
      adapterResult: {
        ok: false,
        errorCode: "adapter_normalizeResult_exception",
        message: "Malformed external response.",
      },
    });
    expect(transitionOperation).toHaveBeenNthCalledWith(
      2,
